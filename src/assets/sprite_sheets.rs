...
impl SpriteOffsets {
    ...
    fn get_bounds_lhand(&self) -> (i32, i32, i32, i32) {
        (self.lhand_x, self.lhand_y, self.lhand_x + 1, self.lhand_y + 1)
    }

    fn get_bounds_rhand(&self) -> (i32, i32, i32, i32) {
        (self.rhand_x, self.rhand_y, self.rhand_x + 1, self.rhand_y + 1)
    }

    fn get_bounds_center(&self) -> (i32, i32, i32, i32) {
        (self.center_x, self.center_y, self.center_x + 1, self.center_y + 1)
    }
    ...
    fn combine_extents((t0, t1, t2, t3): (i32, i32, i32, i32), (b0, b1, b2, b3): (i32, i32, i32, i32)) -> (i32, i32, i32, i32) {
        (
...
pub async fn make_sprite_recolor_sheet(sprite_base_path: &Path) -> Result<CacheBehaviour<Vec<u8>>, anyhow::Error> {
    ...
    let mut combined_img = RgbaImage::new(
        max_size * frame_size_x, max_size * frame_size_y
    );
    ...
    for (idx, (frame, _)) in frames.iter().enumerate() {
        ...
        combined_img.copy_from(
            frame,
            tile_pos_x + diff_pos_x,
            tile_pos_y + diff_pos_y
        )?;
    }
...
async fn get_sprite_frames(sprite_base_path: &Path) -> Result<Vec<(DynamicImage, SpriteOffsets)>, anyhow::Error> {
    ...
    for anim_node in &xml.anims.anim {
        if anim_node.copy_of.is_none() {
            ...
            anim_dims.insert(&anim_node.name, (
                *anim_node.frame_width.as_ref().unwrap() as i32, *anim_node.frame_height.as_ref().unwrap() as i32
            ));
        }
    }
    ...
    for (anim_name, (frame_size_x, frame_size_y)) in anim_dims {
        ...
        for (base_yy, _) in (0..img.height()).step_by(frame_size_y as usize).enumerate() {
            ...
            for xx in (0..img.width()).step_by(frame_size_x as usize) {
                ...
                let (mut bounds_x, mut bounds_y, mut bounds_xm, mut bounds_ym) = get_covered_bounds(&img, tile_bounds);
                let mut missing_tex = if bounds_x >= bounds_xm {
                    (bounds_x, bounds_y, bounds_xm, bounds_ym) = (
                        frame_size_x / 2, frame_size_y / 2,
                        frame_size_x / 2 + 1, frame_size_y / 2 + 1
                    );
                    true
                } else {
                    false
                ...
                let (
                    abs_bounds_x, abs_bounds_y,
                    abs_bounds_xm, abs_bounds_ym
                ) = add_to_bounds((bounds_x, bounds_y, bounds_xm, bounds_ym), (xx, yy));
                let frame_tex = img.crop(
                    abs_bounds_x as u32, abs_bounds_y as u32,
                    (abs_bounds_xm - abs_bounds_x) as u32, (abs_bounds_ym - abs_bounds_y) as u32
                );

                let mut is_dupe = false;
                for (final_frame, final_offset) in &frames {
                    if imgs_equal(final_frame, &frame_tex, false) && offsets_equal(final_offset, &offsets, frame_tex.width() as i32, false) {
                        is_dupe = true;
                        break;
                    }
                    if imgs_equal(final_frame, &frame_tex, true) && offsets_equal(final_offset, &offsets, frame_tex.width() as i32, true) {
                        is_dupe = true;
                        break;
                    }
                }
...
fn offsets_equal(offset1: &SpriteOffsets, offset2: &SpriteOffsets, img_width: i32, flip: bool) -> bool {
...
fn get_covered_bounds(in_img: &DynamicImage, (max_box_x, max_box_y, max_box_xm, max_box_ym): (i32, i32, i32, i32)) -> (i32, i32, i32, i32) {
...
fn add_to_bounds((x, y, xm, ym): (i32, i32, i32, i32), (cx, cy): (i32, i32)) -> (i32, i32, i32, i32) {
    (
        x + cx,
        y + cy,
        xm + cx,
...
fn sub_from_bounds((x, y, xm, ym): (i32, i32, i32, i32), (cx, cy): (i32, i32)) -> (i32, i32, i32, i32) {
    (
        x - cx,
        y - cy,
        xm - cx,
...
fn get_offset_from_rgb(
    ...
    black: bool, r: bool, g: bool, b: bool, white: bool
            ...
            if color[3] == 255 {
                if black && color[0] == 0 && color[1] == 0 && color[2] == 0 {
                    if results[0].is_none() {
                        ...
                        return Err(anyhow!("Multiple black pixels found when searching for offsets!"));
                    }
                }
                if r && color[0] == 255 {
                    if results[1].is_none() {
                        ...
                        return Err(anyhow!("Multiple red pixels found when searching for offsets!"));
                    }
                }
                if g && color[1] == 255 {
                    if results[2].is_none() {
                        ...
                        return Err(anyhow!("Multiple green pixels found when searching for offsets!"));
                    }
                }
                if b && color[2] == 255 {
                    if results[3].is_none() {
                        ...
                        return Err(anyhow!("Multiple blue pixels found when searching for offsets!"));
                    }
                }
                if white && color[0] == 255 && color[1] == 255 && color[2] == 255 {
                    if results[4].is_none() {
                        ...
                        return Err(anyhow!("Multiple white pixels found when searching for offsets!"));
                    }
                }
            }
        }
...
fn get_sprite_frame_size_from_frames(frames: &[(DynamicImage, SpriteOffsets)]) -> (u32, u32) {
    ...
    for (frame_tex, frame_offset) in frames {
        ...
        let (obx, oby, obxm, obym) = frame_offset.get_centered_bounds(
            ((frame_tex.width() / 2) as i32, (frame_tex.height() / 2) as i32)
        );
