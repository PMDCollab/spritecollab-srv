...
impl DatafilesReport {
    pub fn from_data_read_error(file_path: PathBuf, err: DataReadError) -> Self {
        match err {
            DataReadError::SerdeJson(e) => {
                Self::JsonDeserializeError(file_path, e)
            },
            DataReadError::SerdeCsv(e) => {
                Self::CsvDeserializeError(file_path, e)
            },
            DataReadError::Io(e) => {
                Self::IoError(file_path, e)
            },
    ...
    pub fn format_for_discord(&self) -> (&'static str, String) {
        ...
        (
            ...
            match self {
                DatafilesReport::JsonDeserializeError(file_path, err) => {
                    let fname = file_path.file_name().unwrap().to_string_lossy();
                    format!("*{}*\n\n**Description**:\nFailed reading {}: {}{}", DISCORD_UPDATE_INFO, fname, err, self._discord_preview(
                        file_path, err.line()
                    ))
                }
                DatafilesReport::CsvDeserializeError(file_path, err) => {
                    let fname = file_path.file_name().unwrap().to_string_lossy();
                    format!("*{}*\n\n**Description**:\nFailed reading {}: {}{}", DISCORD_UPDATE_INFO, fname, err, self._discord_preview(
                        file_path, err.position()
                            .map_or(0, |p| p.line() as usize)
                    ))
                }
                DatafilesReport::IoError(file_path, err) => {
                    let fname = file_path.file_name().unwrap().to_string_lossy();
                    format!("*{}*\n\n**Description**:\nFailed reading {}: {}", DISCORD_UPDATE_INFO, fname, err)
                }
                DatafilesReport::CreditsDuplicateCreditId(file_path, err) => {
                    let fname = file_path.file_name().unwrap().to_string_lossy();
                    format!("*{}*\n\n**Description**:\nFailed reading {}: {}", DISCORD_UPDATE_INFO, fname, err)
                ...
                DatafilesReport::Ok => {
                    "The SpriteCollab data update is working again.".to_string()
                }
            }
        )
    }
...
pub async fn read_and_report_error<P, FN, FT, R, T>(path: P, generate_fn: FN, reporting: R) -> DataReadResult<T>
where
    P: AsRef<Path> + Copy,
    FN: FnOnce(P) -> FT,
    ...
    R: AsRef<Reporting>
{
    ...
    match &out {
        Ok(_) => {},
        Err(e) => {
            reporting.as_ref().send_event(ReportingEvent::UpdateDatafiles(
