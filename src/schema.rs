...
use juniper::{FieldError, FieldResult, graphql_object, graphql_value, GraphQLEnum, GraphQLObject, GraphQLUnion};
...
use crate::datafiles::credit_names::{CreditNamesRow, parse_credit_id};
...
pub enum Phase {
    ...
    Exists = 1,
    Full = 2,
    #[graphql(description="Returned if the phase value is non-standard. Use phaseRaw to get the raw ID.")]
    Unknown = -1
}
...
pub struct Sprite {
    ...
    offsets_url: String,
    #[graphql(description="URL to the sprite sheet containing the shadow placeholders for each frame.")]
    shadows_url: String
}
...
#[derive(GraphQLUnion)]
#[graphql(description="A single sprite for a single action that is either a copy of another sprite (as defined in the AnimData.xml) or has it's own sprite data.")]
enum SpriteUnion {
    Sprite(Sprite),
    CopyOf(CopyOf),
}
...
#[derive(GraphQLObject)]
#[graphql(description="A SkyTemple Discord Server Guild Point bounty that will be rewarded, if the portrait or sprite has transitioned into a phase.")]
pub struct MonsterBounty {
    #[graphql(description="If true, SpriteBot will not automatically hand out the Guild Point bounty.")]
    modreward: bool,
...
impl MonsterBounty {
    pub fn new(modreward: bool, bounty_spec: &HashMap<i64, i64>) -> Self {
        Self {
            modreward,
            incomplete: bounty_spec.get(&(Phase::Incomplete as i64)).map(|x| *x as i32),
            exists: bounty_spec.get(&(Phase::Exists as i64)).map(|x| *x as i32),
            full: bounty_spec.get(&(Phase::Full as i64)).map(|x| *x as i32),
            other: bounty_spec
                .iter()
                .filter(|(&k, _)| k != (Phase::Incomplete as i64) && k != (Phase::Exists as i64) && k != (Phase::Full as i64))
                .map(|(k, v)| OtherBounty { phase: *k as i32, bounty: *v as i32 })
                .collect()
        }
    }
}
...
impl<'a> MonsterFormPortraits<'a> {
    ...
    fn credit_secondary(&self, context: &Context) -> FieldResult<Vec<Credit>> {
        ...
        self.0.portrait_credit.secondary
            .iter()
            .map(parse_credit_id)
    ...
    fn sheet_url(&self, context: &Context) -> String {
        get_url(AssetType::PortraitSheet, &context.this_server_url, self.1, self.2)
    }

    #[graphql(description="URL to a SpriteBot format recolor sheet.")]
    fn recolor_sheet_url(&self, context: &Context) -> String {
        get_url(AssetType::PortraitRecolorSheet, &context.this_server_url, self.1, self.2)
    }
...
impl<'a> MonsterFormSprites<'a> {
    ...
    async fn fetch_xml_and_make_action_map(anim_xml_url: &str) -> FieldResult<CacheBehaviour<HashMap<String, String>>> {
    ...
    fn failed_xml_fetch<E: Debug>(e: E, anim_xml_url: &str) -> FieldError {
        ...
        FieldError::new(
            "Internal Server Error: Failed processing the animation data from the AnimData.xml.".to_string(),
    ...
    fn get_action_map_blocking(&self, context: &Context) -> FieldResult<HashMap<String, String>> {
        let xml_url = get_url(AssetType::SpriteAnimDataXml, &context.this_server_url, self.1, self.2);
        task::block_in_place(move || {
            Handle::current().block_on(async move {
                context.cached_may_fail_chain(
...
impl<'a> MonsterFormSprites<'a> {
    ...
    fn credit_secondary(&self, context: &Context) -> FieldResult<Vec<Credit>> {
        let names = &context.collab.data().credit_names;
        self.0.sprite_credit.secondary
            .iter()
            .map(parse_credit_id)
            .map(|v| Credit::new(names.get(&v), &v))
            .collect()
    ...
    fn anim_data_xml(&self, context: &Context) -> Option<String> {
        ...
        Some(get_url(AssetType::SpriteAnimDataXml, &context.this_server_url, self.1, self.2))
    }
    ...
    fn zip_url(&self, context: &Context) -> Option<String> {
        ...
        Some(get_url(AssetType::SpriteZip, &context.this_server_url, self.1, self.2))
    }
    ...
    fn recolor_sheet_url(&self, context: &Context) -> Option<String> {
        ...
        Some(get_url(AssetType::SpriteRecolorSheet, &context.this_server_url, self.1, self.2))
    }
...
impl MonsterForm {
    ...
    #[graphql(description="The path to this form (without the monster ID) as it's specified in the SpriteCollab tracker.json file and repository file structure.")]
    fn path(&self) -> String {
    ...
    #[graphql(description="Human-readable full name of this form (excluding the monster name itself).")]
...
impl Monster {
            ...
            Some(collector) => Ok(
                collector
                    .map(|(k, name_path, v)| MonsterForm {
                        id: self.id, form_id: k, name_path, data: v.clone()
                    })
                    .collect()
            ),
    ...
    fn get(&self, context: &Context, form_id: i32, shiny: bool, female: bool) -> FieldResult<Option<MonsterForm>> {
            ...
            Some(collector) => Ok(
                collector
                    .find_form([
                        ...
                        if female { FormMatch::Exact(2) } else { FormMatch::Fallback(1) }
                    ])
                    .map(|(path, name_path, v)| MonsterForm {
                        id: self.id, form_id: path, name_path, data: v.clone()
                    })
            ),
            None => Err(FieldError::new(
                "Monster not found",
    ...
    #[graphql(description="Manually enter the path to a monster, seperated by /. This should match the path as it is stored in SpriteCollab, however the path passed in might be collapsed until a unique form is found.")]
...
pub struct Config {
    ...
    actions: Vec<String>,
    #[graphql(description="Returns a list, that for each phase contains a list of emotions (by index) that need to exist for this phase to be considered completed.")]
    completion_emotions: Vec<Vec<i32>>,
    #[graphql(description="Returns a list, that for each phase contains a list of actions (by index) that need to exist for this phase to be considered completed.")]
    ...
    fn from(c: &SpriteConfig) -> Self {
        Self {
            ...
            action_map: c.action_map
                .iter()
                .map(|(idx, act)| ActionId { id: *idx, name: act.clone() })
                .collect()
        }
    }
}
...
pub struct Credit {
    ...
    id: String,
    #[graphql(description="The human-readable name of the author. Guaranteed to be an ASCII string.")]
    name: Option<String>,
...
impl ScCache for Context {
    ...
    async fn cached_may_fail<S, Fn, Ft, T, E>(&self, cache_key: S, func: Fn) -> FieldResult<Result<T, E>>
        where
            ...
            E: Send
    {
        self.collab.cached_may_fail(cache_key, func).await.map_err(|e| FieldError::new(
...
impl Query {
    ...
    #[graphql(description="Search for a monster by (parts) of its name. Results are sorted by best match.")]
    async fn search_monster(context: &Context, monster_name: String) -> FieldResult<Vec<Monster>> {
        if monster_name.len() > MAX_QUERY_LEN {
            ...
            context.cached_may_fail_chain(
                format!("/search_monster|{}", &monster_name),
                || async {
                    let r: FieldResult<Vec<Monster>> = fuzzy_find_tracker(
                        &tracker,
                        &monster_name,
                        context,
                        |idx| Monster { id: idx as i32 }
                    ).await;
                    match r {
    ...
    #[graphql(
        ...
        arguments(
            filter(
                description = "Monster IDs to limit the request to.",
            )
        ),
    )]
    fn monster(context: &Context, filter: Option<Vec<i32>>) -> FieldResult<Vec<Monster>> {
        Ok(context.collab.data().tracker
            .keys()
            .filter(|v| if let Some(filter) = &filter {
    ...
    #[graphql(description="Search for a credit entry by (parts) of the ID, the author name or the contact info. Results are sorted by best match.")]
    async fn search_credit(context: &Context, query: String) -> FieldResult<Vec<Credit>> {
        if query.len() > MAX_QUERY_LEN {
            ...
            context.cached(
                format!("/search_credit|{}", &query),
                || async {
                    let r: Vec<Credit> = context.collab.data().credit_names
                        .fuzzy_find(&query)
                        .map(Credit::from)
                        .collect();
                    if r.len() > 0 {
    ...
    fn credit(context: &Context) -> FieldResult<Vec<Credit>> {
        Ok(context.collab.data().credit_names
            .iter()
            .map(Credit::from)
            .collect())
    }
