...
fn do_fuzzy_match<S1, S2, II, I>(key: S1, vals_brw: II, query: &S2, matcher: &SkimMatcherV2) -> Option<Vec<(i64, I)>>
where
    S1: AsRef<str>,
    S2: AsRef<str>,
    ...
    I: PrimInt
{
    match matcher.fuzzy_match(&key.as_ref().to_lowercase(), &query.as_ref().to_lowercase()) {
        ...
        Some(score) => if score <= 0 {
            ...
            Some(vals_brw.into_iter().map(|val| (score, val)).collect::<Vec<_>>())
        }
    }
}
...
impl<'a, T: ToOwned + Copy> CloneToVec<T> for Cow<'a, [T]> where [T]: ToOwned
{
    fn clone_to_vec(&self) -> Vec<T> {
        self.to_vec()
    }
